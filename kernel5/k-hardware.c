...
//    the virtual machine; Control-A through Control-F reboot with an
//    indication that the user should run a different program. Returns
//    key typed or -1 for no key.

static void reboot_with_command(const char* command);
...
int check_keyboard(void) {
    ...
    if (c == 0x03 || c == 'q') {
        ...
        reboot_with_command("alice");
    } else if (c == 0x12) {
        reboot_with_command("recurse");
    }
    return c;
}
...
static void reboot_with_command(const char* command) {
    ...
    multiboot_info[0] = 4;
    assert((uintptr_t) command < 0x100000000L);
    multiboot_info[4] = (uint32_t) (uintptr_t) command;
